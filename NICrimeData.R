#Stored the files into the working directory and then reading it into a dataframe 
ni_crime_data <- read.csv("NICrimeData.csv")
#Showing the firts 10 records of the file
head(ni_crime_data)
#Checking the data type of the file
class(ni_crime_data)
#Checking the structure of the file
str(ni_crime_data)
#From the structure displayed, it is evident that, there are a total of 162000 obsevations
#Except the calander year, which of type `integer`, rest of the six variables are of type `factor`
#There are twelve levels for `Calandar year` and `Month`, twenty levels for `Crime Type`, three levels for `Data Measure`
#and three thousand three hundred and twelve levels for count
...
#Checking the levels of `crime type`
levels(ni_crime_data$Crime_Type)

#Shortening the names of crime types
...
#Checking the levels of `Data Measure`
...
#Checking the levels of `Policing District`
levels(ni_crime_data$Policing_District)
#Shortening the names of `Policing District`
attach(ni_crime_data)
...
#Assigning columns having value `0` with `NA`
...
#Adding the dataset umeployement rate of Northern Ireland to the data frame NI
#Reading the data new data set Unemployment Rate
NIUnemp <- read.csv("NIUnempRate.csv")
#Displaying the structure of new NIUnemp data
str(NIUnemp)
#Displaying the column name of NIUnemp
names(NIUnemp)
#Changing the column names of the new dataset
colname_unemp <- c("Year", "Unemp_Rate")
colnames(NIUnemp) <- colname_unemp
unique(NIUnemp$Year)

#Removing rows whete the calander_year does not starts `20`
NIUnemp <- NIUnemp[grepl("20", NIUnemp$Year), , drop = FALSE]

library(tidyr)
NIUnemp <- separate(NIUnemp, Year, into = c("Calendar_Year", "Month"), sep = " (?=[^ ]+$)")

#Removing the Months where quarter names where provided
NIUnemp <- NIUnemp[!grepl("Q", NIUnemp$Month), , drop = FALSE]
#Removing the rows with year `2000`
NIUnemp$Calendar_Year[NIUnemp$Calendar_Year == "2000"] <- NA
NIUnemp$Calendar_Year[NIUnemp$Calendar_Year == "2020"] <- NA
NIUnemp <- NIUnemp[complete.cases(NIUnemp),]
#Removing the first three rows of NIUnemp as those three months fo 
NIUnemp <- NIUnemp[-(1:3), , drop = FALSE]
#Changing the datatype of unemployemnt to numeric
NIUnemp$Unemp_Rate <- as.numeric(paste(NIUnemp$Unemp_Rate))
NIUnemp$Unemp_Rate <- round(NIUnemp$Unemp_Rate)
NIUnemp$Unemp_Rate <- as.numeric(NIUnemp$Unemp_Rate)

#Merging the values of Unemployement Rate column to the data frame Belfast_Crm as new column
Belfast_Crm$Unemp_Rate <- NIUnemp$Unemp_Rate
...
power_information1 <- pwr.r.test(n=225,sig.level=0.05, power=.90)
power_information1

power_information2 <- pwr.r.test(r=.21,sig.level=.05, power=.90)
power_information2

power_information3 <- pwr.r.test(r=.21,n=225,sig.level=.05)
power_information3
